...
function App() {
  ...
  const triggerIntroTransition = () => {
    setZoomActive(true);
  };

  const handleZoomFinished = () => {
    setZoomActive(false); 
    setIsIntro(false); // Matikan mode intro, tampilkan isi web
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-[#020617] text-white selection:bg-cyan-500 selection:text-black">
      ...
      <AnimatePresence mode="wait">
        {isIntro ? (
          /* --- TAMPILAN INTRO AWAL (PRELOADER CINEMATIC) --- */
          <motion.div 
            key="intro-screen"
          ...
          >
            ...
            <div className="pb-16 text-center max-w-sm px-4">
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={triggerIntroTransition}
              ...
              >
                MASUK BERANDA <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
              </motion.button>
              <p className="text-[10px] text-slate-500 mt-4 font-mono">Tekan untuk memicu zoom transisi mata elang</p>
            </div>
          </motion.div>
        ) : (
          /* --- TAMPILAN HALAMAN UTAMA WEB PORTFOLIO --- */
          <motion.div 
            key="main-content"
          ...
          >
            ...
            <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-slate-900/80 backdrop-blur-xl border border-slate-800 px-6 py-3 rounded-2xl flex items-center gap-8 shadow-2xl">
              <button 
                onClick={() => { setIsIntro(true); setZoomActive(false); }}
                ...
                <div className="max-w-5xl mx-auto pt-24 px-4">
                  ...
                  <div className="bg-slate-900/20 border border-slate-800 rounded-xl overflow-hidden">
                    ...
                    <div className="p-4 font-mono text-xs space-y-2.5 text-slate-400">
                      <p><span className="text-cyan-400">[12:03:00]</span> SUCCESS: Mengompilasi transisi kamera mata elang.</p>
                      <p><span className="text-purple-400">[12:03:45]</span> INFO: Efek zoom halaman aktif mendeteksi interaksi pengguna.</p>
                      <p><span className="text-emerald-400">[12:04:10]</span> READY: Struktur Parno Desain Arsitektur siap dieksplorasi.</p>
                    </div>
                  </div>
                </div>
              )}
